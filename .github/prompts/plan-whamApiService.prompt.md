...
**Environment Configuration:** All file paths are driven by environment variables (see `app/core/settings.py`). Paths are loaded from `.env` file (auto-loaded on startup) or OS environment. Copy `.env.example` to `.env` and customize for your deployment; no code changes needed to switch environments.
...
12. Implement upload service flow: validate media, persist metadata, assign stable `video_id`, and store under `${WHAM_DATA_DIR}/videos` (configurable via environment).
...
15. Implement pose3d service mapping from `source_video_id` to server-managed runtime fields: `video`, `output_pth`, `job_id`, `job_name`, `gpu_id`, preprocessing artifact paths (`tracking_results_path`, `slam_results_path`) when available, and execution profile flags (`estimate_local_only`, `visualize`, `save_pkl`, `run_smplify`, `calib`). For API-submitted pose3d jobs, the server profile defaults `visualize=true` unless overridden by a future internal profile change.
16. Encode demo-compatibility rules for pose3d: default global mode, fallback local-only policy when DPVO is unavailable, and output path semantics equivalent to `demo.py` (`output_pth/<video_stem>`). For visualize-enabled pose3d jobs, execution order must be deterministic: preprocessing artifacts first, then 3D inference/visualization.
17. Add deterministic pose3d command builder that generates `docker run` arguments (no raw shell concatenation in handlers), using host data root `${WHAM_DATA_DIR}` (from environment), image `${WHAM_DOCKER_IMAGE}`, mounts for `dataset`, `checkpoints`, `output`, `videos`, and explicit GPU assignment from `${WHAM_GPU_ID}`.
...
20. Implement async pose2d and pose3d GPU-enabled Docker job launchers that return immediately; persist initial `running` state before launch and update to `succeeded`/`failed` on completion monitoring. This async-submit behavior is mandatory for all future inference job types added to the API layer.
...
**Relevant files**
- `${WHAM_REPO_DIR}/demo.py` - authoritative CLI/runtime semantics for `--video`, `--output_pth`, and global/local behavior.
- `${WHAM_REPO_DIR}/wham_api.py` - reusable programmatic flow and output contract expectations.
- `${WHAM_REPO_DIR}/scripts/render_2d_overlay.py` - candidate renderer pipeline for pose2d output generation.
- `${WHAM_REPO_DIR}/scripts/pose2d_artifact_utils.py` - shared utilities for track selection and 2D overlay rendering.
- `${WHAM_REPO_DIR}/configs/config.py` - config precedence patterns to preserve in service defaults.
- `${WHAM_REPO_DIR}/lib/utils/utils.py` - logger/output patterns worth mirroring.
- `${WHAM_REPO_DIR}/.github/skills/wham-api-service/SKILL.md` - required API-layer behavior and Docker mapping policy.
- New folder: `${WHAM_REPO_DIR}/services/wham_api_service/` (FastAPI app, domain service, Docker executor, PostgreSQL persistence, docs/tests). Configuration via `.env` file.
...
- API submit handlers for inference jobs must be non-blocking (detached dispatch) and return immediately after successful launch.
- State storage: PostgreSQL (not in-memory/SQLite/Redis).
- New service location: `services/wham_api_service` with environment-driven configuration (`.env` file).
- API includes upload/download plus pose2d/pose3d jobs and association query.
- Runtime flags remain server-owned.
- For pose3d jobs received from API layers, default runtime profile sets `visualize=true`.
